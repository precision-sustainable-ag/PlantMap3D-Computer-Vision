...
with dai.Device(pipeline) as device:
    ...
    for photo in photos_in:
        ...
        out_file = photo.split(".")[0]
        out_dir = photos_out_dir + out_file + ".png"
        ##ERROR: data_o is depthai.NNData type, not a format for cv2 image
        print(data_o.shape)
        #cv2.imwrite(out_dir,data_o)
        time.sleep(0.5)